import sendMail, { falciMailTemplate, userMailTemplate } from "../../helpers/mailer.js";
...
function FalEndPoints(app, connection) {
    ...
    app.post('/api/insertUserFalRequest', authenticateToken, async (req, res) => {
        ...
        try {
            ...
            const getEmailAddresSql =
                "SELECT" +
                "    U.email, " +
                "    CASE WHEN U.user_role = 2 THEN 'FALCI' ELSE 'KULLANICI' " +
                "END AS TYPE " +
                "FROM " +
                "  fals F " +
                "JOIN users U ON " +
                "    F.user_id = U.id OR F.fal_user_id = U.id " +
                "WHERE " +
                "    F.id = ? ";

            var falciEmailAdress = '';
            var userEmailAdress = '';

            try {
                const rows = await connection.raw(getEmailAddresSql, [fal_id]);

                if (rows.length > 0) {
                    rows.map((item) => {
                        if (item.length > 0) {
                            item.map((item2 => {
                                if (item2.TYPE == 'KULLANICI') {
                                    userEmailAdress = item2.email;
                                }
                                else if (item2.TYPE == 'FALCI') {
                                    falciEmailAdress = item2.email;
                                }
                            }))
                        }
                    })

                    if (falciEmailAdress != '' && userEmailAdress != '') {
                        sendMail(falciEmailAdress, "Fortuneteller Bildirim", falciMailTemplate(falciEmailAdress, app_time));
                        sendMail(userEmailAdress, "Fortuneteller Bildirim", userMailTemplate(userEmailAdress, app_time));
                    }
